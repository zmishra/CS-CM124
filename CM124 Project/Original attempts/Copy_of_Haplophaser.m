...
%     phased_partition = zeros(size(B, 1), 2*size(B, 2));
    ...
    all_haplotypes = containers.Map;
    %all_haplotypes = [];
    gen_haplotypes = cell(size(A, 2),1);
    gen_freq = cell(size(A, 2), 1);
    snp = 1;
    while  size(keys(all_haplotypes), 2) < 20000 %size(all_haplotypes, 2) < 20000
        all_haplotypes = containers.Map;
        ...
        for i = 1:size(A, 2)
            ...
            switch A(snp, i)
                case 1
                    gen_haplotypes{i} = [gen_haplotypes{i} gen_haplotypes{i}];
                    while size(hetero, 2) < size(gen_haplotypes{i},2)
                        hetero = [hetero ~hetero];
                    end
                    gen_haplotypes{i}(snp, :) = hetero;
                case 2
                    gen_haplotypes{i}(snp,:) = 1;
                case 0
                    gen_haplotypes{i}(snp,:) = 0;
            end
            
            for j = 1:size(gen_haplotypes{i}, 2)
                all_haplotypes(num2str(gen_haplotypes{i}(:,j))) = 1;
            end
            
%             if i == 1
%                 all_haplotypes = gen_haplotypes{i};
%             else
%                 all_haplotypes = unique([all_haplotypes gen_haplotypes{i}].', 'rows').';
%             end
        end
        snp = snp + 1;
    end
...
%         else
%             freq = rand(1,size(gen, 2)/2);
%             gen_freq{i} = freq/sum(freq);
%         end
        all_haplotypes1 = unique([all_haplotypes1 gen].', 'rows').';
    end
...
%   P = 1/size(all_haplotypes,2)*ones(size(all_haplotypes,2),1);
    ...
    while max(abs(P-P_old)) > 10^-3 && sum(blacklist) ~= size(blacklist, 1)
...
toc
dlmwrite('20exp2.txt', phased_population, 'delimiter', ' ');
