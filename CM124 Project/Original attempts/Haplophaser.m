...
%Load data
% A = importdata('example_data_1.txt', ' ');
...
for start_ = 1:SNP_Partition:size(A,1)
    ...
    all_haplotypes = [];
    gen_haplotypes = cell(size(B, 2),1);
    gen_freq = cell(size(B, 2), 1);
    for i = 1:size(B, 2)
        hetero = [1 0];
    ...
    for i = 1:1:size(all_haplotypes, 2)
        allhapcells{i} = char(string(all_haplotypes(:,i)));
    end
    ...
    %maxtracker=zeros(size(gen_haplotypes,1),1);
    while iterations < 6 %max(abs(P-P_old)) > 10^-3 && sum(blacklist) ~= size(blacklist, 1)
...
%             if blacklist(i) == 0
            ...
            if size(curr_gen, 2) ~= 1
                for j = 1:2:size(curr_gen, 2)
                    hap1 = curr_gen(:,j);
                    hap2 = curr_gen(:,j+1);
                    index_hap1 = hapMap(char(string(hap1)));
                    index_hap2 = hapMap(char(string(hap2)));
...
%                 end
                ...
                index_hap1 = hapMap(char(string(curr_gen(:,1))));
                P_gen(index_hap1) = 2;
%                 index_P_max(i,1) = index_hap1;
...
%             end
            ...
            P_new = P_new + P_gen;
            %maxtracker(i) = max(P_gen);
        end
        P = P_new/size(gen_haplotypes,1)/2;
        
%         new_gen_freq = cell(size(B, 2), 1);
        for i = 1:size(gen_haplotypes,1)
            curr_gen = gen_haplotypes{i};
            curr_gen_freq = gen_freq{i};
            new_curr_gen_freq = zeros(1,size(curr_gen_freq, 2));
            lambda = zeros(1,size(curr_gen_freq, 2));
%             S = zeros(ceil(size(curr_gen, 2)/2)); 
            ...
            for j = 1:2:size(curr_gen, 2)
                ...
                P_hap1 = P(index_hap1);
                P_hap2 = P(index_hap2);
                new_curr_gen_freq((j+1)/2) = P_hap1*P_hap2;
                lambda((j+1)/2) = P_hap1 + P_hap2;
            end
            Si = sum(new_curr_gen_freq);
            new_curr_gen_freq = new_curr_gen_freq/Si;
            
            %Determine lambda
            lambda_cnt = sum(new_curr_gen_freq - curr_gen_freq > 0);
            lambda_sum = 0;
            upper_bounds = -curr_gen_freq./(new_curr_gen_freq - curr_gen_freq);
            upper_bounds(upper_bounds <= 0) = MAX_LAMBDA;
            upper_bound = min(upper_bounds);
            for j = 1:size(curr_gen_freq, 2)
                if new_curr_gen_freq(j) - curr_gen_freq(j) >= 0
                    lambda_temp = 1 + size(curr_gen_freq, 2)/(Si/lambda(j)- size(curr_gen_freq, 2));
                    if lambda_temp < 0
                        lambda_temp = 1;
                    end
                    lambda_sum = lambda_sum+lambda_temp;
                end
            end
            lambda_sum/lambda_cnt
            lambda_avg = max(1, min(lambda_sum/lambda_cnt, upper_bound));
            gen_freq{i} = curr_gen_freq+lambda_avg*(new_curr_gen_freq-curr_gen_freq);
        end
    end
...
toc
dlmwrite('17exp3bl.txt', phased_population, 'delimiter', ' ');
